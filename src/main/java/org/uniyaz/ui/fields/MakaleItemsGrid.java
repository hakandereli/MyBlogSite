...
public class MakaleItemsGrid extends GridLayout {
    ...
    public MakaleItemsGrid(Kategori kategori) {
        MyUI myUI = (MyUI) UI.getCurrent();
        this.myContent = myUI.getMyContent();

        this.kategori = kategori;
        allMakaleByKategori = makaleService.findAllMakaleByKategori(kategori);
        satirSayisi = Math.ceil(Double.valueOf(allMakaleByKategori.size()) / 3.0);
        if (allMakaleByKategori.size() > 0) {
...
//            makaleItem.setWidth(33f,Unit.PERCENTAGE);
            setRowExpandRatio(count,32f);
            makaleItem.setHeight("500");
            makaleItem.setMargin(true);
